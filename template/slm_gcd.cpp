...
ll f(ll a,ll b,ll c,ll n) ///�������$Sigma_{i=0}^{n} floor((a*i+b)/c)$
{
    ll m = (a*n+b)/c;
...
ll g(ll a,ll b,ll c,ll n) ///�������$Sigma_{i=0}^{n} i*floor((a*i+b)/c)$
{
    ll m = (a*n+b)/c;
    if(n==0||m==0) return 0;
...
ll f_sqr(ll a,ll b,ll c,ll n,ll r)  ///�������$Sigma_{i=1}^{n} floor(i*(a*sqrt(r)+b)/c)$
{
    double w = sqrt(r);
    if(n==0) return 0;
